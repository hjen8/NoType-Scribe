...
def apply_dictionary_post_process(text: str) -> str:
    ...
    text = text.replace("金要", "金鑰")
    text = text.replace("常住程式", "常駐程式")
    text = text.replace("更號", "根號")
    text = re.sub(r'進程(?=管理|列表|清單|ID|排程|處理)', '行程', text)
...
def generate_notes(transcript: str, on_model_switch=None, app_mode: str = 'general') -> str:
    ...
    system_prompt = (
        ...
        "12. 【Top 50 臺灣高頻同音多義互斥語境特徵矩陣 (Context-Aware Collocation Matrix)】：\n"
        "    [類別一：公務法制合約 (10組)]\n"
        "    1. 制定(法律/憲法/法規/國家政令/立法院正式創制) vs 制訂(計畫/方案/辦法/草案/細則/合約訂立)\n"
        "    2. 反映(民意/意見/問題/陳情/心聲/反饋) vs 反應(化學/熱烈反響/物理/生理/遲鈍)\n"
        "    3. 公權力(國家執法/統治力量/公權力介入) vs 權利(公民權利/言論自由/自身權益/法律利益)\n"
        "    4. 法制(法規體制/法制體系/法制局) vs 法治(依法治理/法治國家/落實法治社會)\n"
        "    5. 登錄(實價登錄/成績登錄/名冊登記) vs 登入(帳號登入/密碼驗證/系統進入)\n"
        "    6. 申覆(成績複查/採購異議審查/提出申覆) vs 申訴(學生申訴/性平申訴/申訴管道)\n"
        "    7. 核定(主管機關核定拍板/核定通過) vs 核訂(重新審核訂定標準/費率)\n"
        "    8. 終止(契約終止/終止合約/永久結束) vs 中止(中途暫停/比賽因雨中止/暫時停頓)\n"
        "    9. 廢止(法定處分廢止/依法廢止許可) vs 廢除(廢除舊制/廢除死刑/革除制度)\n"
        "    10. 賦予(抽象給予：賦予權限/使命/意義) vs 付與(交付金錢：付與款項/實體交付)\n"
        "    [類別二：日常高頻易混 (15組)]\n"
        "    11. 及(並列連接詞：名詞及名詞、老師及學生) vs 即(副詞：即日起/立即/即使/也就是即是)\n"
        "    12. 副(副校長/副組長/副作用/一副眼鏡) vs 附(附件/附帶/附註/附近商圈/依附)\n"
        "    13. 做(具體動手動作/做好/做飯/做人/做事/做筆記) vs 作(抽象創作/作文/作品/作業/合作/作用/作為)\n"
        "    14. 在(地點/存在/正在進行/在家/在學校) vs 再(時間延續/又一次/重新再來/再見/再次/再說一次)\n"
        "    15. 須(助動詞：必須/須知/無須/務須) vs 需(動詞名詞：需要/需求/民生必需品)\n"
        "    16. 度(溫度/長度/角度/態度嚴謹/制度健全/過度) vs 渡(渡河/渡過難關/渡輪/過渡期)\n"
        "    17. 帳(帳號/帳本/記帳/結帳/查帳) -> 臺灣一律採用「帳」，絕不用「賬」！\n"
        "    18. 座(名詞：座位/星座/講座/一座山) vs 坐(動詞：請坐/坐下/坐車/坐下來談)\n"
        "    19. 制(制度/限制/控制/壓制/法制) vs 致(導致/致敬/致詞/致力/專心致志)\n"
        "    20. 意(意思/意願/意見/心意/故意) vs 義(意義/正義/義工/字義/義大利)\n"
        "    21. 象(大象/現象/氣象/想像/形象) vs 相(互相/真相/相片/相當/丞相)\n"
        "    22. 部(部門/部分/教育部/全體) vs 步(步驟/腳步/散步/進步)\n"
        "    23. 辨(辨識/分辨/明辨是非) vs 辦(辦理/辦公室/辦法/舉辦) vs 辯(辯論/爭辯/答辯)\n"
        "    24. 絕(絕對/絕不/杜絕/絕望) vs 決(決定/決策/解決/決心)\n"
        "    25. 報(報告/報導/報名/報答) vs 暴(暴力/暴風雨/暴躁/風暴)\n"
        "    [類別三：教學學術數學 (10組)]\n"
        "    26. 全對(全部答對/考卷小明竟然全對/答案全對/滿分/這頁全對/讚美答題) vs 全隊(球隊/中華隊全隊集合/全隊團結)\n"
        "    27. 中點(幾何線段中點/中點坐標/三角形中點) vs 終點(馬拉松終點/抵達終點線/競賽目標)\n"
        "    28. 人名(姓名/名單/稱呼/字典人名/學生人名/這一段人名/其他部分的人名) vs 人民(國家人民/國民/百姓/人民權利)\n"
        "    29. 多項式(數學多項式/一元二次多項式/多項式除法) vs 多項是(列舉多個項目是)\n"
        "    30. 質數(因數/質數判斷/最小質數/質數分解) vs 指數(指數函數/指數律/物價指數/股票指數)\n"
        "    31. 交集(集合交集聯集/人生交集) vs 焦急(心情急迫/焦急等待)\n"
        "    32. 根號(數學根號/開根號/根號二) vs 更號(更改號碼/更改座號)\n"
        "    33. 係數(多項式係數/方程式係數/相關係數) vs 細數(仔細列舉/細數回憶/不勝細數)\n"
        "    34. 截距(坐標軸截距/直線斜率與截距/截距式) vs 差距(距離差別/貧富差距/城鄉差距)\n"
        "    35. 題綱(課程綱要/命題大綱/考試題綱) vs 提綱(提綱挈領/演講提綱)\n"
        "    [類別四：電腦資訊軟硬體 (10組)]\n"
        "    36. 程式(軟體/代碼/腳本/寫程式/執行/運作/常駐) vs 城市(都會/市區/臺北市/街道)\n"
        "    37. 主選單(軟體操作/右鍵選單/系統匣/功能表) -> 100% 輸出「主選單」，絕不可輸出同音字「竹選單」！\n"
        "    38. 磁碟(電腦儲存/硬碟/S磁碟/隨身碟/RAMDISK) -> 100% 輸出「磁碟」，絕不可輸出「詞疊」！\n"
        "    39. 金鑰(API Key/安全金鑰/私鑰/密碼金鑰) -> 100% 輸出「金鑰」，絕不輸出「金要」！\n"
        "    40. 常駐(背景常駐/常駐程式/常駐系統匣/Daemon) vs 常住(常住人口/常住居民)\n"
        "    41. 端點(API Endpoint端點/網路端點/音訊裝置端點) vs 斷點(程式偵錯中斷點/中斷)\n"
        "    42. 行程(臺灣繁體 Process 稱「行程」或「執行緒」；勿用大陸用語進程！)\n"
        "    43. 記憶體(臺灣繁體 RAM 稱「記憶體」；勿用大陸用語內存！)\n"
        "    44. 伺服器(臺灣繁體 Server 稱「伺服器」；勿用大陸用語服務器！)\n"
        "    45. 相依(套件相依性/相依函式庫/依賴相依庫) vs 相互(互相照應/相互合作)\n"
        "    [類別五：口語連音同化盲區 (5組)]\n"
        "    46. 真因(根本原因/排查真因/問題真因) vs 基因(生物遺傳/DNA/基因突變)\n"
        "    47. 心聲(內心想法/傾聽心聲/基層心聲/說出心聲) vs 新生(新生訓練/報到/入學/新生兒)\n"
        "    48. 屆時(到時候/預定時間抵達：屆時請準時出席) -> 100% 輸出「屆時」，絕不輸出「借時」！\n"
        "    49. 給予(給予支持/給予協助/給予評價) -> 臺灣慣用「給予」！\n"
        "    50. 涵蓋(範圍包含/課題涵蓋/教材涵蓋五大單元) -> 優先輸出「涵蓋」！\n"
