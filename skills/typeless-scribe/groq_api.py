...
def generate_notes(transcript: str, on_model_switch=None, app_mode: str = 'general') -> str:
    ...
    system_prompt = (
        ...
        "   - 核心原則：徹底濾除說話時無意識的思考停頓詞與口吃打結字，讓輸出的文字乾淨俐落！\n"
        "   - 思考停頓詞清剿（一律強制刪除，絕不輸出）：『嗯、啊、呃、那個、這個、就是說、對啊、其實』若出現在句首作為純思考發語詞或句中停頓，【一律強制剔除】！\n"
        "   - ★【嚴禁誤刪『然後』】：『然後』是中文中極為重要的承接連詞（表示下一步、接續動作、指示交辦、邏輯遞進，例如：『然後再重列一次給我』、『先看 A，然後再看 B』）！除非同一句內無意識連續結巴重複出現（如『然後然後』），否則正常的『然後』【一律強制 100% 完整保留】，絕對不可以擅自刪除或吞噬！\n"
        "   - 口吃打結去重：字詞重複打結（如『我…我想說』->『我想說』；『這個這個問題』->『這個問題』；『然後然後』->『然後』）必須全自動合成為單一正確詞。\n"
        "   - 去贅字對比範例：\n"
        "     * 範例 1：『呃…那個…其實我們今天要討論的…嗯…就是高二的課程。』-> 輸出『我們今天要討論高二的課程』\n"
        "     * 範例 2：『然後再重列一次給我。』-> 輸出『然後再重列一次給我』\n"
        ...
        "11. 【常見電腦與檔案系統專用詞彙 100% 規範】：\n"
        "    - 「磁碟」：特別在『S磁碟、C磁碟、磁碟機、RAMDISK』等電腦語境，100% 輸出「磁碟」或「S碟」，絕不輸出「詞疊」！\n"
        "    - 「主選單」：在軟體介面、右鍵、系統匣操作語境下，100% 輸出「主選單」，絕不可輸出同音字「竹選單」！\n"
        "    - 「金鑰」：在 API Key、密碼金鑰語境下，100% 輸出「金鑰」，絕不輸出「金要」！\n"
        "12. 【Top 8 核心語意雙向消歧義矩陣 (Core Homophone Collocation Matrix)】：\n"
        "    1. 全對(考試答題全部答對/小明竟然全對/答案全對/滿分) vs 全隊(球隊集合/中華隊全隊一心)\n"
        "    2. 中點(幾何線段中點/三角形中點/坐標中點) vs 終點(馬拉松終點/抵達終點線/競賽目標)\n"
        "    3. 人名(親友學生人名名單/輸入人名/稱呼/這一段人名) vs 人民(國家人民/國民百姓/人民權益)\n"
        "    4. 程式(撰寫程式/執行程式/常駐程式/Python代碼) vs 城市(臺北市城市/都會市區/鄉鎮聚落)\n"
        "    5. 做(具體動手動作/做筆記/做好/做飯/做人/動手實作) vs 作(抽象創作/作文/作品/作業/作為/作用/合作)\n"
        "    6. 在(地點所在/正在進行/在家/在學校) vs 再(時間延續/又一次/再來一次/再見/再說一次)\n"
        "    7. 及(並列連接詞：老師及學生、長官及同仁) vs 即(副詞/等同：即日起/立即/也就是即是)\n"
        "    8. 度(溫度/長度/角度/態度嚴謹/制度健全/過度) vs 渡(渡河/渡船/渡過難關/過渡時期)\n"
